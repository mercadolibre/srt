...
class Dataset(torch.utils.data.Dataset):
    ...
    def _data_processing(self):
        ...
        remap_dt = self.config['remap_dataset']
        if remap_dt is not None:
            self._remap_from_other_dt(remap_dt)
        else:
            self._remap_ID_all()
        ...
        for field in self._rest_fields:
            ...
            if field == self.uid_field:
                ...
                if not isinstance(source, str):
                    source = source.value
                df = getattr(self, f"{source}_feat")

            tok_map = remap_dt.field2token_id[field]
            if field_type == FeatureType.TOKEN:
                df[field] = df[field].apply(lambda x: tok_map.get(x, 0))
            elif field_type == FeatureType.TOKEN_SEQ:
                df[field] = df[field].apply(lambda x: [tok_map[e] for e in x if e in tok_map])

        for field, token_id_map in remap_dt.field2token_id.items():
            if field not in self.field2token_id:
                self.field2token_id[field] = copy.deepcopy(token_id_map)

        for field, id_token_map in remap_dt.field2id_token.items():
            if field not in self.field2id_token:
                self.field2id_token[field] = copy.deepcopy(id_token_map)
    ...
    def _change_feat_format(self):
        ...
        for feat_name in self.feat_name_list:
            ...
            if isinstance(feat, Interaction): continue
    ...
    def join(self, df):
        ...
        if self.item_feat is not None and self.iid_field in df:
            # TODO: this only works with Interaction, not DataFrame
