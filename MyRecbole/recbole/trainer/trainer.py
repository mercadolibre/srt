...
class Trainer(AbstractTrainer):
    ...
    def _build_scheduler(self, steps_per_epoch):
        ...
        if scheduler_cfg['type'] == 'one-cycle':
            return torch.optim.lr_scheduler.OneCycleLR(
                ...
                final_div_factor=scheduler_cfg.get('final_div_factor', 0.1)
            )
    ...
    def _build_optimizer(self, **kwargs) -> Optimizer:
        ...
        if hasattr(self.model, 'get_learning_rates') and self.config['differential_learning_rates']:
            params = self.model.get_learning_rates(learning_rate)
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False, total_batches=None):
        ...
        Args:
            ...
            total_batches (int): train only for that number of batches. Useful to estimate memory consumption
        ...
        if hasattr(self.model, 'on_epoch_start'):
            ...
            self.metrics_logger.log_metric('time_on_epoch_start', time() - t0, epoch_idx)

        scaler = amp.GradScaler(enabled=self.enable_scaler)
        gradient_accumulation_steps = self.config["gradient_accumulation_steps"] or 1
        n_batches = 0
        t0 = time()
        for batch_idx, interaction in enumerate(iter_data):
            ...
            loss = scaler.scale(loss + sync_loss)
            loss += (self.loss_callback(self.model) or 0)
            loss.backward()
            ...
            if batch_idx % gradient_accumulation_steps == 0:
                ...
                if self.gpu_available and show_progress:
                    ...
                    iter_data.set_custom_msg(
                        set_color("GPU RAM: " + get_gpu_usage(self.device) + f' loss: {total_loss / n_batches if n_batches else 0:.05f}', "red")
                    )

            if self.scheduler:
                self.scheduler.step()

        self.metrics_logger.log_metric('time_epoch', time() - t0, epoch_idx)
    ...
    def _fit(
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            if (epoch_idx + 1) % self.eval_step == 0:
                ...
                tr_result = self.evaluate(
                    FullSortEvalDataLoader(train_data.config, train_data.dataset, valid_data.sampler),
                    phase='train', load_best_model=False,
                    show_progress=show_progress, neg_sample_func=train_data._neg_sampling,
                    n_batches=len(valid_data)
                )
    ...
    def _sampled_sort_batch_eval(self, batched_data, n_negatives):
        ...
        try:
            ...
            scores = self.model.sampled_predict(interaction, n_negatives)
    ...
    def evaluate(
        ...
        sanity_check=False, sampled_n=None, n_batches=None
    ):
        r"""Evaluate the model based on the eval data.
        ...
        iter_data = (
            progress(
                ...
                tot=len(eval_data),
                # ncols=100,
                desc=set_color(f"Evaluate in {phase}   ", "pink"),
            )
            if show_progress
            else eval_data
        )
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            if n_batches is not None and batch_idx >= n_batches: break
            n_batch += 1

            # TODO: Move interaction to device. Make sure this works consistent with other evaluation methods
            interaction, history_index, positive_u, positive_i = batched_data
            interaction = interaction.to(self.device)
            ...
            if phase != 'train':
                ...
                with torch.autocast(device_type=self.device.type, enabled=self.enable_amp):
                    losses = loss_func(neg_sample_func(interaction))
        ...
        if phase != 'train':
            result[f'{phase}_loss'] = total_loss / n_batch if n_batch else 0
        ...
        if not sanity_check:
            ...
            loss_callback_val = self.loss_callback(self.model)
            if loss_callback_val is not None:
                self.metrics_logger.log_metric('loss_callback', loss_callback_val.item(), self.cur_epoch)

        return result

    def load_checkpoint(self, model_file=None):
